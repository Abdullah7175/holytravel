  ...
  const generateFullClientPDF = async (bookingId: string) => {
    try {
      ...
      const addFooter = () => {
        ...
        doc.setFont('helvetica', 'bold');
        doc.text('HOLY TRAVEL', margin, footerY + 15);
      ...
      doc.save(`HOLY-TRAVEL-Booking-${full.id}.pdf`);
    } catch (e: any) {
  ...
  const generateInvoicePDF = async (bookingId: string) => {
    try {
      ...
      try {
        ...
        console.log('Could not load holytravel.png for invoice:', logoError);
      ...
      // Save the PDF
      doc.save(`HOLY-TRAVEL-Invoice-${invoiceNo}.pdf`);
    } catch (e: any) {
